...
// bool hfs_ctp_adaptor::onResponse(int teid, std::string execid, hfs_order_t* order) {
...
void hfs_ctp_adaptor::OnRspAuthenticate(CThostFtdcRspAuthenticateField *pRspAuthenticateField, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
    LOG_INFO("[ctp adaptor] OnRspAuthenticate");
    finished = success = true;
}
...
void hfs_ctp_adaptor::OnRtnOrder(CThostFtdcOrderField *pOrder)
{
  ...
  if(pOrder->OrderSubmitStatus == THOST_FTDC_OSS_InsertSubmitted) {
    // smap[enode->sid] = enode;
    // nmap[enode->nid] = enode;    
    order.qty = enode->qty;
    order.type = 'A';
    order.state = ORD_NEW;    
  }
...
int hfs_ctp_adaptor::Login() {
  ...
  if(m_state != CTP_LOGGEDON) return -1;
  ...
  finished = success = false;
  ret = pUserApi->ReqSettlementInfoConfirm(&nreq, ++iRequestID);
  if(ret != 0) {
    LOG_ERROR("请求结算错误");
    return -1;
  }
  ...
  LOG_INFO("CTP logged in successfully");
...
void hfs_ctp_adaptor::qryPosition() {
  ...
  //等待持仓请求成功
  for(;!finished;) { sleep(10); }
  // dpi->get_data_group().add_data("holdings", holdings);
...
void hfs_ctp_adaptor::qryInstrument() {
  ...
  //等待请求成功
  for(;!finished;) { sleep(10); }
  LOG_INFO("CTP Qry InstrumentInfo done");
}
...
void hfs_ctp_adaptor::printFtdcOrder(CThostFtdcOrderField *pOrder) {
  LOG_INFO("[tradingtool] OnRtnOrder:instrumentID:{}, OrderRef:{}, VolumeTotalOriginal:{}, VolumeTrade:{}, StatusMsg:{}, OrderStatus:{}",
             pOrder->InstrumentID, pOrder->OrderRef, pOrder->VolumeTotalOriginal, pOrder->VolumeTraded,
             pOrder->StatusMsg, (char)pOrder->OrderStatus);
}
